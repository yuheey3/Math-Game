...
const llv1hit = JSON.parse(localStorage.getItem("Lv1hit"));
const llv1miss = JSON.parse(localStorage.getItem("Lv1miss"));
const ltime = JSON.parse(localStorage.getItem("localTime"));
...
hit1 = localhit;
mis = localmiss;
lv1count_hit = llv1hit;
lv1countmiss = llv1miss;
let lName = 0;
let lAge = 0;
...
let saveinfo = () => {
    ...
    localStorage.setItem("saveLv1hit", lv1count_hit);
    localStorage.setItem("saveLv1miss", lv1countmiss);
    localStorage.setItem("levelStage", 1);
...
function wrong() {
    ...
    score1.innerHTML = `${numscore}`;
    miss.innerHTML = `${mis}`;
    //when the miss is 6, game is over
    if (mis === 6) {
